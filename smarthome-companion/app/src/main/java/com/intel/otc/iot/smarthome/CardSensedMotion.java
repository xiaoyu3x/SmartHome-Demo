...
public class CardSensedMotion extends CardOcResource {
    private static final String TAG = CardSensedMotion.class.getSimpleName();
    public  static final String RESOURCE_TYPE = "oic.r.sensor.motion";
    private static final String KEY_VALUE = "value";
    private static final int imageAlphaMotionSensed = 255, imageAlphaNoMotion = 30;

    private TextView mResourceId;
    private ImageView mIcon;

    CardSensedMotion(View parentView, Context context) {
        ...
        mIcon.setImageResource(R.drawable.motion);
    }
    ...
    private void updateState(OcRepresentation rep) {
        try {
            ...
            ((Activity)mContext).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    mIcon.setImageAlpha(state? imageAlphaMotionSensed : imageAlphaNoMotion);
                }
            });
        } catch (OcException e) {

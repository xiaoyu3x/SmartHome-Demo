...
char *gResourceUri= (char *)"/a/fan";       // the URI of the Resource
...
void IOT_Setup(){ 
    ...
    IOT_.Type = (char *)"boxFan";   // Sets the Type for Ruls
    IOTsleep = 3;
}
...
OCRepPayload* sendPayload()
{
    ...
    OCRepPayloadSetPropBool(att_payload, "on_off", IOT_.on_off);
    OCRepPayloadSetPropInt(att_payload, "power", IOT_.power);
...
void recivedPayload(OCRepPayload* input){
    ...
    bool on_off;
    if(OCRepPayloadGetPropBool(input, "on_off", &on_off))
    {
        IOT_.on_off = on_off;
    }

# -*- coding: utf-8 -*-
"""
CRUD operation for power model
"""
from DB.api import database
from DB import exception
from DB.models import Power
...
def new(session, src_dic, content={}):
    ...
    return utils.add_db_object(session, Power, **content)


def _get_power(session, gateway_id, uuid, order_by=[], limit=None, **kwargs):
    ...
    return \
        utils.list_db_objects(session, Power, order_by=order_by, limit=limit, gateway_id=gateway_id, uuid=ids, **kwargs)


@database.run_in_session()
@utils.wrap_to_dict(RESP_FIELDS)  # wrap the raw DB object into dict
def get_solar_by_gateway_uuid(session, gateway_id, uuid):
    return _get_power(session, gateway_id, uuid)
...
def get_latest_by_gateway_uuid(session, gateway_id, uuid, ):
    solar = _get_power(session, gateway_id, uuid, order_by=[('id', True)], limit=1)

...
def _get_power(session, resource_id, order_by=[], limit=None, **kwargs):
    ...
    return \
        utils.list_db_objects(session, Power, order_by=order_by, limit=limit, resource_id=resource_ids, **kwargs)


@database.run_in_session()
@utils.wrap_to_dict(RESP_FIELDS)  # wrap the raw DB object into dict
def get_solar_by_gateway_uuid(session, resource_id):
    return _get_power(session, resource_id)
...
def get_latest_by_gateway_uuid(session, resource_id, ):
    solar = _get_power(session, resource_id, order_by=[('id', True)], limit=1)

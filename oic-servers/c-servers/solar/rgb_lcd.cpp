...
{
    i2c->address(LCD_ADDRESS);        // transmit to device #4
    i2c->writeByte(dta);                            // sends five bytes
...
void rgb_lcd::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) 
{
    ...
    sleep(0.45);  // wait more than 4.1ms
...
void rgb_lcd::clear()
{
    ...
    sleep(0.2000);          // this command takes a long time!
}
...
{
    i2c->address(RGB_ADDRESS); // transmit to device #4

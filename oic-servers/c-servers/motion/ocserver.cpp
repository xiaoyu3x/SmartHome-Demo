...
char *gResourceUri= (char *)"/a/pir";       // the URI of the Resource
...
void IOT_Setup(){ 
    ...
    IOT_.Type = (char *)"motionSensor";   // Sets the Type for Ruls
    IOTsleep = 1;
...
void IOT_Loop(){
    ...
    if(IOT_.on_off != m_motion){

        IOT_.on_off = m_motion;//set some data
        IOT_.hasUpdate = true;  // will send new data to IOT
    }
}
...
OCRepPayload* sendPayload()
{
    ...
    OCRepPayloadSetPropInt(att_payload, "on_off", IOT_.on_off);
